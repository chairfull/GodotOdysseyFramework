...
func _input_keyboard(_ev: InputEvent) -> void:
	var cont := get_controller()
	if cont.is_action_pressed(&"move_left") or cont.is_action_pressed(&"move_forward"):
		button_list.hover_next(-1)
	elif cont.is_action_pressed(&"move_right") or cont.is_action_pressed(&"move_backward"):
		button_list.hover_next(1)
...
func select() -> void:
	var buttons: TweeButtonList = %button_list
	if buttons.select():
		var choice := choices[buttons.hovered]
		if "call" in choice:
			(choice.call as Callable).call()
