@abstract class_name WorldBase extends Node
...
class StatEvent extends Event:
	...
	var increased: bool:
		get: return new > old
	var reverted: bool:
		get: return new == stat.default

class AwardEvent extends Event:
	var award: AwardInfo

#region Events.
var PAWN_CONTROLLED := PawnEvent.new()
var PAWN_UNCONTROLLED := PawnEvent.new()
var PAWN_MOUNTED := PawnEvent.new()
var PAWN_UNMOUNTED := PawnEvent.new()
...
var QUEST_DISCOVERED := QuestEvent.new()
...
func change_scene(id: StringName) -> void:
